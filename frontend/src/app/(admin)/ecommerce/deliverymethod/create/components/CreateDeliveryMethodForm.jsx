...
import { createDeliveryMethod, updateDeliveryMethod } from '@/http/DeliveryMethod';

const CreateDeliveryMethodForm = ({ isEditMode, initialData, methodId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await updateDeliveryMethod(methodId, formData);
      ...
      toast.error(error.response?.data?.error || `Failed to ${isEditMode ? 'update' : 'create'} delivery method`);
    }
  };
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="d-flex justify-content-end gap-2 mt-4">
        <button
          ...
          onClick={() => navigate('/ecommerce/deliverymethods')}
        >
          Cancel
        </button>
        <button type="submit" className="btn btn-success">
          {isEditMode ? 'Update' : 'Create'} Delivery Method
...
export default CreateDeliveryMethodForm;
