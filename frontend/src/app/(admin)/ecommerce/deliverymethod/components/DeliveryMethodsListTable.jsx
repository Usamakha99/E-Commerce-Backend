...
import { deleteDeliveryMethod } from '@/http/DeliveryMethod';
import toast from 'react-hot-toast';
import { Badge } from 'react-bootstrap';

const DeliveryMethodsListTable = ({ methods, loading, onDelete }) => {
  const handleDelete = async (id, name) => {
    if (window.confirm(`Are you sure you want to delete the delivery method "${name}"?`)) {
      try {
        ...
        toast.error(error.response?.data?.error || 'Failed to delete delivery method');
      }
    }
  };

  const handleEdit = (id) => {
    window.location.href = `/ecommerce/deliverymethods/create?edit=${id}`;
  };
        ...
        <div className="d-flex align-items-center">
          <div className="flex-grow-1">
            <h5 className="mt-0 mb-1">
              <Link
                to={`/ecommerce/deliverymethods/${id}`}
  ...
  return <ReactTable columns={columns} data={methods} />;
};

export default DeliveryMethodsListTable;
